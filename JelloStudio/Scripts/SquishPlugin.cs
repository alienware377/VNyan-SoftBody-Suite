...
namespace JelloStudio
{
    ...
    public class SquishPlugin : MonoBehaviour, VNyanInterface.IButtonClickedHandler
    {
        ...
        void SetupWindow()
        {
            ...
            {
                cageWholeToggle = cw;
                cw.onValueChanged.AddListener(v =>
                {
                    if (suppress || config.settings == null) return;
                    config.settings.cageBindWhole = v;
                    SetStatus(v ? "clothes now bind to the WHOLE body surface (re-binding…)"
            ...
            if (co != null)
            {
                clothOutsideToggle = co;
                co.onValueChanged.AddListener(v =>
                {
                    if (suppress || config.settings == null) return;
                    config.settings.cageClothOutside = v;
                    SetStatus(v ? "clothes will be held off the skin (cloth moves, body untouched)"
                                : "cloth clearance guard off");
                });
            }
        ...
        void PushRegionToUI()
        {
            ...
            if (cageWholeToggle != null && config.settings != null) cageWholeToggle.isOn = config.settings.cageBindWhole;
            Slider rsz;
            ...
            { rsz.value = config.settings.cageInflate; SetValueLabel("cageinflate", rsz.value); }
            if (config.settings != null && sliders.TryGetValue("cageinflatedyn", out rsz) && rsz != null)
            { rsz.value = config.settings.cageInflateDyn; SetValueLabel("cageinflatedyn", rsz.value); }
        ...
        HashSet<Transform> CollectRegionBones()
        {
            ...
            for (int p = 0; p < proxies.Count; p++)
            {
                ...
                if (names.Length > 0)
                    Debug.Log("[Jello] scoped native override bones (" + px.smr.name + "): " + names);
            }
            return set;
        }
        ...
        Toggle cageWholeToggle;
        Toggle clothOutsideToggle;
