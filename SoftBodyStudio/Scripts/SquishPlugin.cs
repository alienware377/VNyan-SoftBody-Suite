...
{
    // SoftBody Studio — mesh-level soft-body deformation with live weight painting.
    ...
    {
        const string BUTTON_NAME = "SoftBody Studio";
        const string CONFIG_FILE = "softbodystudio.json";
        ...
        void Update()
        {
            ...
            if (Input.GetKeyDown(KeyCode.F10))
            {
                ...
                Debug.Log("[SoftBody] collider debug " + (MeshProxy.debugDraw ? "ON" : "OFF"));
        ...
        void Awake()
        {
            ...
            catch (Exception e) { Debug.LogWarning("[SoftBody] registerPluginButton failed: " + e.Message); }
            savePath = Path.Combine(Application.persistentDataPath, CONFIG_FILE);
            LoadConfig();
            SyncRegionsFromOwner(true);
            SetupWindow();
            Debug.Log("[SoftBody] initialized. Config: " + configPath);
        }
        ...
        void EnsureAvatar()
        {
            ...
            Rebind();
            Debug.Log("[SoftBody] bound to avatar '" + av.name + "'");
        }
        ...
        void Rebind()
        {
            ...
            for (int m = 0; m < config.meshes.Count; m++)
            {
                ...
                if (target == null) { Debug.LogWarning("[SoftBody] mesh '" + sm.mesh + "' not found"); continue; }
        ...
        void SetupWindow()
        {
            ...
            catch (Exception e) { Debug.LogWarning("[SoftBody] instantiateUIPrefab failed: " + e.Message); window = null; }
            if (window == null) return;
            ...
            AddTooltips(new Dictionary<string, string>
            {
                ...
                { "Button_Save", "Write everything to softbodystudio.json (also syncs regions to Wobble Studio)." },
        ...
        void ApplyNativeOverride()
        {
            ...
            Debug.Log("[SoftBody] native physics override: " + disabled + " disabled"
        ...
        void SaveConfigQuiet()
        {
            ...
            catch (Exception e) { Debug.LogWarning("[SoftBody] quiet save failed: " + e.Message); }
        }
        ...
        void SyncRegionsFromOwner(bool force)
        {
            try
            {
                ...
                if (changed)
                {
                    ...
                    Rebind();
                    RefreshMeshList();
                    Debug.Log("[SoftBody] regions mirrored from Squish Studio");
                }
            }
            catch (Exception e) { Debug.LogWarning("[SoftBody] region mirror failed: " + e.Message); }
        }
        ...
        void LoadConfig()
        {
            ...
            catch (Exception e) { Debug.LogWarning("[SoftBody] config load failed: " + e.Message); config = new SquishConfig(); }
