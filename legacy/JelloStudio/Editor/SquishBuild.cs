...
// Builds the Jello Studio window prefab + starter prefab and packs them into the
...
public static class SquishBuild
{
    ...
    public static void Build()
    {
        ...
        const string starterPrefabPath = "Assets/JelloStudioStarter.prefab";
        const string bundleName = "jellostudio_bundle";
        ...
        JelloStudio.SquishPlugin plugin = go.AddComponent<JelloStudio.SquishPlugin>();
        plugin.windowPrefab = windowAsset;
        // opaque double-sided vertex-color overlay material (bundled with the prefab)
        Shader ovSh = AssetDatabase.LoadAssetAtPath<Shader>("Assets/JelloOverlay.shader");
        if (ovSh != null)
        {
            Material ovMat = new Material(ovSh);
            AssetDatabase.CreateAsset(ovMat, "Assets/JelloOverlayMat.mat");
            plugin.overlayMaterial = ovMat;
        }
        else Debug.LogWarning("[SquishBuild] JelloOverlay.shader not found — overlay falls back to Sprites/Default");
        ...
        string final = Path.Combine(outDir, "JelloStudio.vnobj");
    ...
    static GameObject BuildWindowPrefab(string prefabPath)
    {
        ...
        root.AddComponent<JelloStudio.SquishWindowDrag>();

        float hy = 10f;
        MakeText(root.transform, "Title", "Jello Studio — DETACHED Soft Body (experimental)",
