...
namespace SoftBodySuite.Wobble
{
    ...
    using SquishRegion = SoftBodySuite.WobbleRegion;
    ...
    public class MeshProxy
    {
        ...
        // ---- in-suite chain ----
        // Wobble runs first, so it is the one that bakes; it only ever hands arrays on.
        ...
        // this stage's own contribution, which the clothes need to see
        public Vector3[] Disp { get { return disp; } }
        ...
        public void Attach(SkinnedMeshRenderer target, SquishMesh meshCfg, GameObject avatar, Animator anim)
        {
            ...
            for (int r = 0; r < cfg.regions.Count; r++)
            {
                ...
                s.Build(cfg.regions[r].wobble, display, weldOf, weldMembers);
        ...
        public void ResolveColliderMeshes(GameObject avatar)
        {
            ...
            for (int r = 0; r < cfg.regions.Count; r++)
                for (int c = 0; c < cfg.regions[r].wobble.colliders.Count; c++)
                {
                    string nm = cfg.regions[r].wobble.colliders[c].mesh;
        ...
        void UpdateColliderClouds()
        {
            ...
            foreach (KeyValuePair<string, SkinnedMeshRenderer> kv in colMeshSmr)
            {
                ...
                if (self && selfExclude == null)
                {
                    ...
                    for (int r = 0; r < cfg.regions.Count; r++)
                        for (int v = 0; v < cfg.regions[r].wobble.vertIndex.Count; v++)
                            if (cfg.regions[r].wobble.weight[v] > 0.15f) selfExclude.Add(cfg.regions[r].wobble.vertIndex[v]);
                }
                ...
                for (int r = 0; r < cfg.regions.Count; r++)
                    for (int c = 0; c < cfg.regions[r].wobble.colliders.Count; c++)
                        if (cfg.regions[r].wobble.colliders[c].mesh == kv.Key && cfg.regions[r].wobble.colliders[c].radius > rad)
                            rad = cfg.regions[r].wobble.colliders[c].radius;
                cloud.Build(rad);
            }
