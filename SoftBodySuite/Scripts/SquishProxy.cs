...
namespace SoftBodySuite.Squish
{
    ...
    public class MeshProxy
    {
        ...
        public void Attach(SkinnedMeshRenderer target, SquishMesh meshCfg, GameObject avatar, Animator anim)
        {
            ...
            for (int r = 0; r < cfg.regions.Count; r++)
            {
                ...
                s.Build(cfg.regions[r].squish, display, weldOf, weldMembers);
                BuildEvacClusters(s, cfg.regions[r].squish);
        ...
        public void ResolveColliderMeshes(GameObject avatar)
        {
            ...
            for (int r = 0; r < cfg.regions.Count; r++)
                for (int c = 0; c < cfg.regions[r].squish.colliders.Count; c++)
                {
                    string nm = cfg.regions[r].squish.colliders[c].mesh;
        ...
        void UpdateColliderClouds()
        {
            ...
            for (int r = 0; r < cfg.regions.Count; r++)
            {
                SquishRegion reg = cfg.regions[r].squish;
            ...
            foreach (KeyValuePair<string, SkinnedMeshRenderer> kv in colMeshSmr)
            {
                ...
                if (self && selfExclude == null)
                {
                    ...
                    for (int r = 0; r < cfg.regions.Count; r++)
                        for (int v = 0; v < cfg.regions[r].squish.vertIndex.Count; v++)
                            if (cfg.regions[r].squish.weight[v] > 0.15f) selfExclude.Add(cfg.regions[r].squish.vertIndex[v]);
                }
                ...
                for (int r = 0; r < cfg.regions.Count; r++)
                    for (int c = 0; c < cfg.regions[r].squish.colliders.Count; c++)
                        if (cfg.regions[r].squish.colliders[c].mesh == kv.Key && cfg.regions[r].squish.colliders[c].radius > rad)
                            rad = cfg.regions[r].squish.colliders[c].radius;
                cloud.radius = rad;
        ...
        public static void SelectFromBonesOn(SkinnedMeshRenderer smr, SoftBodySuite.SuiteRegion region,
        ...
        public List<string> DeriveRegionBones(SoftBodySuite.SuiteRegion region, float shareCutoff)
        {
        ...
        public void RefreshOverlayColors(SoftBodySuite.SuiteRegion region)
        {
        ...
        public void SelectFromBone(SoftBodySuite.SuiteRegion region, string boneName, float threshold, bool includeChildren)
        {
        ...
        public static void SelectFromBoneOn(SkinnedMeshRenderer smr, SoftBodySuite.SuiteRegion region,
        ...
        public List<Vector4> RegionWorldSamples(SoftBodySuite.SuiteRegion region)
        {
        ...
        public static int TransferWeights(SkinnedMeshRenderer target, SoftBodySuite.SuiteRegion region,
        ...
        // The guard used to run inline in the middle of the frame, measuring against a
        // garment the cloth pass had moved a frame EARLIER. It now runs at the very end of
        // the suite, after the clothes have been fitted to the finished body this frame,
        // and re-publishes only what it changed.
        public void RunClipGuard()
        {
            if (!Alive) return;
            EnforceClipGuard();
            display.SetVertices(bakedVerts);
        ...
        {
            if (settingsRef == null || !settingsRef.cloth.guard.enabled) { if (clipBound) ClipGuardInvalidate(); return; }
            if (bakedVerts == null || weldOf == null) return;
            float range = Mathf.Clamp(settingsRef.cloth.guard.range, 0.005f, 0.3f);
            float clear = Mathf.Clamp(settingsRef.cloth.guard.clearance, 0f, 0.05f);
            float strength = Mathf.Clamp01(settingsRef.cloth.guard.strength);
            float rimFade = Mathf.Max(0f, settingsRef.cloth.guard.rimFade);
        ...
        public void BlurRegion(SoftBodySuite.SuiteRegion region, float amount)
        {
        ...
        public bool PaintStroke(SoftBodySuite.SuiteRegion region, Ray worldRay, float radius, float strength, int mode)
        {
