...
namespace SoftBodySuite.Jello
{
    ...
    public class RemeshCage
    {
        ...
        public static float[] UnionWeights(int mv, List<SoftBodySuite.SuiteRegion> regions, int[] weldOf)
        {
            float[] wRep = new float[mv];
            bool anyW = false;
            for (int r = 0; r < regions.Count; r++)
            {
                SoftBodySuite.SuiteRegion reg = regions[r];   // paint only, no stage numbers
